...
if (!exists("result_file_cohort")) {
  result_file_cohort <- paste0(OUTPUT_DIR_BASE, "/outputLocal/", PROJECT_NAME, "/Kohorte.csv")
}

if(!file.exists(result_file_cohort)){
  write("Ergebnisse nicht gefunden.", file = error_file)
  stop("Results not found - aborting.")
}

# read result csv
cohort_report <- read.csv(result_file_cohort, sep = ";", colClasses = "character")
